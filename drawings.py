...
def live_dead(v):
    ...
    def section_panel(cut, along_x):
        ...
        scale, x_px, y_px, close, _width = _ground_shape_frame(radius, total, units, below=below_grade)
        body = f'<polygon points="{_polygon_str(_pile_outline(core, stem_wall, dome_height), x_px, y_px)}" {PILE}/>'
        ...
        if along_x:
            for depth in (-hopper_drop, -below_grade):
                body += (
                    f'<line x1="{x_px(-radius)}" y1="{y_px(depth)}" '
                    f'x2="{x_px(radius)}" y2="{y_px(depth)}" {STRUCT_THIN}/>'
                )
            for side in (-radius, radius):
                body += (
                    f'<line x1="{x_px(side)}" y1="{y_px(-hopper_drop)}" '
                    f'x2="{x_px(side)}" y2="{y_px(-below_grade)}" {DASHED}/>'
                )
        else:
            for t_c in sorted({ocy for _ocx, ocy, _w, _l in openings}):
                r_px = tunnel_r * scale
                body += (
                    f'<path d="M{x_px(t_c - tunnel_r)} {y_px(-below_grade)} '
                    f'L{x_px(t_c - tunnel_r)} {y_px(-hopper_drop - tunnel_r)} '
                    f'A{r_px:.1f} {r_px:.1f} 0 0 1 {x_px(t_c + tunnel_r)} {y_px(-hopper_drop - tunnel_r)} '
                    f'L{x_px(t_c + tunnel_r)} {y_px(-below_grade)} Z" {STRUCT_THIN}/>'
                )
